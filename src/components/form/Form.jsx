...
const Form = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-2 sm:p-4">
      <div className="max-w-3xl w-full p-4 sm:p-6 border border-gray-300 rounded bg-white">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col sm:flex-row gap-2 mt-2">
            <button
              type="button"
              onClick={addRow}
              className="bg-green-500 hover:bg-green-600 text-white text-lg font-extrabold px-3 py-1 rounded cursor-pointer w-full sm:w-auto"
            >
              +
            </button>
            ...
            <button
              type="submit"
              className="bg-blue-500 hover:bg-blue-600 text-white text-lg font-medium px-3 py-1 rounded mt-2 sm:mt-0 w-full sm:w-auto"
            >
              Submit
            </button>
          </div>
            ...
            {rows.map((row) => {
              ...
              return (
                <h3 key={row.id} className="text-lg text-gray-600">
                  Row {row.id + 1} : Name : {data.name || ""}, Role :{" "}
                  {data.role || ""}
                </h3>
              );
            })}
        ...
        {Object.keys(submittedData).length > 0 && (
          <div className="mt-6 overflow-x-auto">
            ...
            <table className="table-auto border-collapse border border-gray-300 w-full min-w-[400px]">
              <thead>
                <tr>
